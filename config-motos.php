<?php 
	...
	$sql="SELECT cat_nombre FROM cat_motos WHERE cat_id=".$cat_id;
		...
		<article>
			...
			<div id="panel-nuevos" style="background-image: url(logos/body.jpg);background-repeat: no-repeat;">
				<div class="main-atajo">
					...
					<a href="index.php">Inicio » </a><span>Motos </span>
			...
			?>
			    ...
			    <div class="update-motos">
				<?php 
				while ($fila=mysqli_fetch_assoc($resultado)) {
				 ?>
				

					<div class="motos-view-edit">
						<img src="imgMotos/<?php echo $fila['foto']; ?>" alt="">
						<h3><?php echo $fila1["cat_nombre"]," ",$fila["modelo"]; ?></h3>
						<b>$ <?php echo number_format($fila["precio"],3); ?></b>
						<p><?php echo $fila["anio"]," | ",$fila["km"]," km | ",$fila["cilindrada"]; ?></p>
						<a class="link-conf-u espaciado-1" href="form-editar-motos.php?id_motos=<?php echo $fila["id_motos"]; ?>">Editar</a>
						<a class="link-conf-u espaciado-2" href="form-eliminar-motos.php?id_motos=<?php echo $fila["id_motos"]; ?>">Eliminar</a>
					</div>
				<?php 
				}
				?>
